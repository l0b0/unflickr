...
class Unflickr:
    ...
    def get_photo_list(self, date_low, date_high):
        ...
        while True:
            ...
            index = index + 1
            rsp = self.fapi.photos_search(api_key=self.flickr_api_key,
    ...
    def get_geotagged_photo_list(self):
        ...
        while True:
            if self.verbose:
                print 'Requesting a page...'
            index = index + 1
            rsp = self.fapi.photos_getWithGeoData(api_key=self.flickr_api_key,
                                                  auth_token=self.token,
                                                  user_id=self.flickr_user_id,
                                                  per_page=str(flickr_max),
                                                  page=str(index))
    ...
    def get_photo_location(self, pid):
        ...
        rsp = self.fapi.photos_geo_getLocation(api_key=self.flickr_api_key,
                                               auth_token=self.token,
                                               photo_id=pid)
    ...
    def __download_report_hook(self, count, block_size, total_size):
    ...
    def download_url(self, url, target, filename):
...
def usage():
    ...
    print """unflickr - Version %(version)s
Backs up Flickr photos and metadata
Usage: unflickr.py -i <flickr Id>
...
def file_write(dryrun, directory, filename, string):
...
def backup_photo(index, total, uid, title, target, hash_level, unflickr,
    ...
    if is_video:
        ...
        try:
            ...
            print 'warning: private video %s cannot be downloaded due to'
            print 'a Flickr bug' % str(uid)
...
def backup_photos(threads, unflickr, target, hash_level, date_low, date_high,
    ...
    for photo in photos:
        ...
        if threads > 1:
            ...
            downloader = PhotoBackupThread(concurrent_threads,
                                           index,
                                           total,
                                           pid,
                                           photo['title'],
    ...
    for photoset in photosets:
        ...
        if photos == None:
            ...
            for photo in photos:
                ...
                target_dst = target + '/sets/' + pid + '/photos'
                if threads > 1:
                    concurrent_threads.acquire()
                    downloader = PhotoBackupThread(concurrent_threads,
                                                   index,
